if getgenv().LoadedScript then
	pcall(function()
		if getgenv().Shutdown then
			getgenv().Shutdown()
		end
	end)
end

getgenv().LoadedScript = true

pcall(function()
	local plrGui = game.Players.LocalPlayer:FindFirstChild("PlayerGui")
	if plrGui then
		for _, gui in ipairs(plrGui:GetChildren()) do
			if gui.Name == "main" or gui.Name == "SettingsGui" or gui.Name == "PositionDisplayGui" then
				gui:Destroy()
			end
		end
	end
...
getgenv().Shutdown = function()
	pcall(function()
		if type(_G.StopForceFieldLoop) == "function" then _G.StopForceFieldLoop() end
		if type(_G.StopPlayerListLoop) == "function" then _G.StopPlayerListLoop() end
		if type(_G.StopTPInstantLoop) == "function" then _G.StopTPInstantLoop() end
		if type(_G.ResetAntiEscape) == "function" then _G.ResetAntiEscape() end
		if type(_G.StopESPPlayerLoop) == "function" then _G.StopESPPlayerLoop() end
...
Frame.Parent                         = main
local LeaveVPortal               = Instance.new("TextButton")
...
LeaveVPortal.Name = "LeaveVPortal"
LeaveVPortal.Parent = Frame
LeaveVPortal.BackgroundColor3 = Color3.fromRGB(79, 255, 152)
LeaveVPortal.Size = UDim2.new(0, 44, 0, 28)
LeaveVPortal.Font = Enum.Font.SourceSans
LeaveVPortal.Text = "LEAVE"
LeaveVPortal.TextColor3 = Color3.fromRGB(0, 0, 0)
LeaveVPortal.TextSize = 17
...
LeaveVPortal.MouseButton1Click:Connect(function()
	local char = LocalPlayer.Character
	if not char then return end
...
local function readEscapeConfig()
	...
	local sp = tonumber(spTeleportBox.Text)
	return y or 500000, sp or 450
end
...
EscapeStart = function()
	...
	-- Sử dụng Heartbeat thay cho RenderStepped để xử lý vật lý mượt mà hơn
	escapeConn = RunService.Heartbeat:Connect(function(dt)
		if not escapeEnabled or not escapeActive then return end
		...
		local targetY, speed = readEscapeConfig()
		...
		if currentPos.Y < targetY then
			local nextY = currentPos.Y + (speed * dt)
			
			if nextY > targetY then
				nextY = targetY
			end
			
			hrp.CFrame = CFrame.new(currentPos.X, nextY, currentPos.Z)
		else
...
local VirtualFoundation = Slots[5]
...
local SpeedForce = Slots[4]
SpeedForce.Label.Text = "Speed"
...
local TeleportPlayerInstant = Slots[15]
...
local AntiEscape = Slots[3]
AntiEscape.Label.Text = "Lock Player"
AntiEscape.Label.TextColor3 = Color3.fromRGB(230,50,50)
AntiEscape.Frame.ClipsDescendants = true

-- Khử trùng lặp giao diện con nếu có chạy lại script
if AntiEscape.Frame:FindFirstChild("rowAntiEscape") then
	AntiEscape.Frame.rowAntiEscape:Destroy()
end

-- Tạo khung chứa ô Radius
local rowAntiEscape = Instance.new("Frame")
rowAntiEscape.Name = "rowAntiEscape"
rowAntiEscape.Parent = AntiEscape.Frame
rowAntiEscape.Size = UDim2.new(1, -12, 1, -8)
rowAntiEscape.Position = UDim2.fromOffset(120, 4) 
rowAntiEscape.BackgroundTransparency = 1
rowAntiEscape.ZIndex = 20

local layoutAntiEscape = Instance.new("UIListLayout")
layoutAntiEscape.Parent = rowAntiEscape
layoutAntiEscape.FillDirection = Enum.FillDirection.Horizontal
layoutAntiEscape.VerticalAlignment = Enum.VerticalAlignment.Center
layoutAntiEscape.Padding = UDim.new(0, 1)

-- Tạo TextBox thay đổi bán kính ranh giới
local radiusBox = Instance.new("TextBox")
radiusBox.Parent = rowAntiEscape
radiusBox.Size = UDim2.fromOffset(40, 28)
radiusBox.Text = "1900"
radiusBox.PlaceholderText = "Radius"
radiusBox.ClearTextOnFocus = false
radiusBox.Font = Enum.Font.SourceSansBold
...
-- Các biến cấu hình nội bộ
local sphereRadius = 1900
local MIN_RADIUS = 0
local MAX_RADIUS = 50000
local PUSH_BACK_OFFSET = 20 

local lockConn = nil 
local myDeathConn = nil 
local targetDeathConn = nil 

local lockAllowed = true

-- Lắng nghe sự kiện thay đổi bán kính trong TextBox
radiusBox:GetPropertyChangedSignal("Text"):Connect(function()
	local filtered = radiusBox.Text:gsub("%D", "")
	if filtered == "" then
		radiusBox.Text = ""
		return
	end
	local num = math.clamp(tonumber(filtered), MIN_RADIUS, MAX_RADIUS)
	sphereRadius = num
	radiusBox.Text = tostring(num)
end)

-- Hàm xử lý kẹp thẳng tọa độ khi vượt biên
local function updateSphereLock()

	if not AntiEscape.State then return end
	...
	local directionVector = currentPos - targetPos
	local currentDistance = directionVector.Magnitude

	if currentDistance > sphereRadius then
		myHRP.AssemblyLinearVelocity = Vector3.zero
		myHRP.AssemblyAngularVelocity = Vector3.zero

		local safeDistance = sphereRadius - PUSH_BACK_OFFSET
		local newPos = targetPos + (directionVector.Unit * safeDistance)
		
		myHRP.CFrame = CFrame.new(newPos) * myHRP.CFrame.Rotation
	end
end

-- HÀM TẮT HỆ THỐNG & ĐƯA PILL VỀ OFF (Khi click tắt HOẶC khi có người chết/reset)
local function disableLock()
	...
	-- Ép trạng thái State và đưa giao diện Pill về OFF hoàn toàn
	AntiEscape.State = false

	AntiEscape.Pill.BackgroundColor3 =
	Color3.fromRGB(80,80,80)

	AntiEscape.SlotKnob:TweenPosition(
		UDim2.fromOffset(2,2),
		Enum.EasingDirection.Out,
		Enum.EasingStyle.Quad,
		0.15,
...
local function enableLock()
	...
	-- Theo dõi nếu bản thân Reset / Chết -> Gọi hàm tự tắt Pill về OFF
	local myChar = LocalPlayer.Character
	local myHum = myChar and myChar:FindFirstChildOfClass("Humanoid")
	if myHum then
		myDeathConn = myHum.Died:Connect(function()
			disableLock()
		end)
	end

	-- Theo dõi nếu mục tiêu Reset / Chết -> Gọi hàm tự tắt Pill về OFF
	...
	if targetHum then
		...
		targetDeathConn = targetHum.Died:Connect(function()
			disableLock()
		end)
	end

	-- Vòng lặp chính kiểm tra khoảng cách ranh giới
	lockConn = RunService.Heartbeat:Connect(function()

		if not AntiEscape.State then return end

		lockAllowed = updateHPState(lockAllowed)

		if not lockAllowed then
			disableLock()
			return
		end
...
-- SỰ KIỆN CLICK PILL
AntiEscape.Pill.MouseButton1Click:Connect(function()

	lockAllowed =
	updateHPState(lockAllowed)
	...
	AntiEscape.State =
	not AntiEscape.State

	if AntiEscape.State then
...
-- Hàm global hỗ trợ dọn dẹp khi reload menu
_G.ResetAntiEscape = function()
	disableLock()
end
...
local function updateESP()    
    ...
    for player, data in pairs(espCache) do    
        ...
        if health >= maxHealth then
            healthFormat = string.format("[ %.0f/%.0f ]", health, maxHealth)
        else
            healthFormat = string.format("[ %.5f/%.0f ]", health, maxHealth)
        end
        
        local newText = string.format("<b>%s [ %.0fm ]</b>\n%s", player.Name, distance, healthFormat)    
    
        if newText ~= data.lastText then    
            data.lastText = newText    
            data.label.Text = newText    
            data.label.TextColor3 = getHealthColor(percent)    
        end    
    end    
end
