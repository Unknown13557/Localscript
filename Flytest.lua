...
CatchPlayer.Label.Text = "Tracking Player"
...
tpBtn.Size = UDim2.fromOffset(155, 28)
tpBtn.Text = "Teleport tracker"
tpBtn.Font = Enum.Font.SourceSansBold
tpBtn.TextSize = 14
tpBtn.TextColor3 = Color3.fromRGB(240,240,240)
...
local NoclipSlot = Slots[16]
NoclipSlot.Label.Text = "Noclip"
NoclipSlot.Frame.ClipsDescendants = true

local function disableNoclipSlot()

	removeNoclipUser()

	if NoclipSlot.State then

		NoclipSlot.State = false

		NoclipSlot.Pill.BackgroundColor3 =
		Color3.fromRGB(80,80,80)

		NoclipSlot.SlotKnob:TweenPosition(
			UDim2.fromOffset(2,2),
			Enum.EasingDirection.Out,
			Enum.EasingStyle.Quad,
			0.15,
...
NoclipSlot.Pill.MouseButton1Click:Connect(function()

	NoclipSlot.State =
	not NoclipSlot.State
...
local function applyHitbox(player, localHRP)
	...
	if originalStates[player]
		and originalStates[player].ref ~= hrp then
...
BindSlot(HitboxExpand, {
	...
	Callback = function(dt, char, hum, hrp)
		...
		applyHitbox(selectedPlayer, hrp)
