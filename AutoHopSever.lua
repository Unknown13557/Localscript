...
-- Danh sách đen lưu các server đã thử hoặc full
local blacklistedServers = {}
blacklistedServers[game.JobId] = true
...
-- Hàm Auto Hop Server hỗ trợ lật trang + Chờ 1s giữa mỗi lần thử
local function autoHopServer()
	local req = (syn and syn.request) or (http and http.request) or http_request or request
	local cursor = ""

	for page = 1, 5 do
		local url = "https://games.roblox.com/v1/places/" .. tostring(game.PlaceId) .. "/servers/Public?sortOrder=Asc&limit=100"
		if cursor ~= "" then
		...
		if success and result and result.data then
			for _, server in ipairs(result.data) do
				if type(server) == "table" and server.id and not blacklistedServers[server.id] then
					if server.playing and server.playing < server.maxPlayers then
						blacklistedServers[server.id] = true -- Đánh dấu server
						...
						-- Nghỉ 1 giây trước khi thử server tiếp theo trong danh sách
						task.wait(1)
					end
				end
			end
	...
	-- Dự phòng: Teleport ngẫu nhiên
	return pcall(function()
		TeleportService:Teleport(game.PlaceId, player)
	end)
end

-- Bắt sự kiện thất bại từ Roblox
TeleportService.TeleportInitFailed:Connect(function(plr, teleportResult, errorMessage)
	if plr == player then
		teleporting = true
		show(0, 0)
		task.wait(1) -- Chờ 1s trước khi gọi Hop lại
		task.spawn(function()
			autoHopServer()
		end)
	end
	...
	while true do
		if enabled and not teleporting then
			if remaining > 0 then
				...
				task.spawn(function()
					while enabled and teleporting do
						local success = autoHopServer()
						if success then break end
						task.wait(1) -- Chờ 1 giây nếu lượt thử trước không vào được server nào
					end
				end)
			end
		else
